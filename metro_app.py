...
# ── Aktif tren hesaplama (sistem durumu için de kullanılır) ───────────────────

def aktif_trenler_hesapla(istasyonlar, seferler, toplam_sure, simdi):
    n      = len(istasyonlar)
    dk_bas = toplam_sure / (n - 1) if n > 1 else toplam_sure
    sure   = [i * dk_bas for i in range(n)]
    aktif  = []
    for kalkis in seferler:
        bitis = kalkis + timedelta(minutes=toplam_sure)
...
def hat_durumu_hesapla(hat, tarih, simdi):
    """Tek bir hat için tüm yönlerin aktif trenlerini döndürür."""
...
def api_sistem_durumu():
    hatlar = ibb_get("GetLines")
    for h in hatlar:
        h["renk"] = HAT_RENKLERI.get(h.get("Name",""), "#555")
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        gelecekler = {executor.submit(hat_durumu_hesapla, h, tarih, simdi): h for h in hatlar}
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Sistem durumu */
  .sistem-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 14px; }
  .hat-card { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
  .hat-card-header { padding: 12px 16px; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid var(--border); }
  .hat-badge { padding: 4px 12px; border-radius: 20px; font-weight: 700; font-size: .85rem; color: #fff; }
  .hat-card-body { padding: 12px 16px; }
  .yon-blok { margin-bottom: 12px; }
  .yon-blok:last-child { margin-bottom: 0; }
  .yon-baslik { font-size: .7rem; color: var(--muted); text-transform: uppercase; letter-spacing: .8px; margin-bottom: 6px; }
  .mini-tren { display: flex; align-items: center; gap: 8px; padding: 6px 0; border-bottom: 1px solid var(--border); font-size: .82rem; }
  .mini-tren:last-child { border-bottom: none; }
  .mini-kalkis { font-weight: 600; min-width: 40px; color: var(--muted); font-size: .75rem; }
  .mini-seg { flex: 1; display: flex; align-items: center; gap: 6px; }
  .mini-ok { color: var(--accent); font-size: .8rem; }
  .mini-kalan { font-size: .72rem; color: var(--muted); min-width: 36px; text-align: right; }
  .hat-yukleniyor { padding: 16px; text-align: center; color: var(--muted); font-size: .8rem; }
  .hata-mesaj { padding: 10px; font-size: .78rem; color: #f87171; }
  .bos-mesaj { padding: 10px; font-size: .78rem; color: var(--muted); text-align: center; }
  ...
  .sistem-meta { font-size: .8rem; color: var(--muted); }
  /* Responsive */
...
<div class="container">
  ...
  <div class="panel" id="panel-ozellik">
    ...
    <div style="margin-top:12px">
      <div class="btn btn-feature" onclick="sistemDurumu()" style="background:rgba(232,66,27,.08);border-color:var(--accent)">
        <div class="icon">🚇</div>
        <div class="label" style="color:var(--accent)">Tüm Sistem Durumu</div>
        <div class="desc">Bütün hatlarda trenler şu an nerede?</div>
      </div>
    </div>
  </div>
  ...
  <div class="panel" id="panel-sistem">
    <div class="back" onclick="goster('ozellik')">← Geri</div>
    <div class="refresh-row">
      <div class="sistem-meta" id="sistem-meta">Yükleniyor...</div>
      <button class="refresh-btn" onclick="yukleSimdi()">↻ Yenile</button>
    </div>
...
async function yukleSimdi() {
  ...
  meta.textContent = "Veriler çekiliyor... (18 hat, ~15 sn)";

  // İskelet kartlar göster
  ...
  grid.innerHTML = hatlar.map(h => `
    <div class="hat-card" id="hcard-${h.Id}">
      <div class="hat-card-header">
        ...
        <span style="font-size:.8rem;color:var(--muted)">yükleniyor...</span>
      </div>
      <div class="hat-card-body hat-yukleniyor"><div class="spinner" style="width:18px;height:18px;border-width:2px"></div></div>
    </div>
  `).join("");

  // Tüm veriyi tek çağrıda al (backend paralel işler)
  const veri = await get("/api/sistem-durumu");
  meta.textContent = `🕐 ${veri.simdi}  ·  ${veri.tarih}  ·  ${veri.hatlar.length} hat`;
  ...
  veri.hatlar.forEach(hat => {
    ...
    let icerik = "";
    if (hat.hata) {
      icerik = `<div class="hata-mesaj">⚠ Veri alınamadı</div>`;
    } else if (!hat.yonler || hat.yonler.length === 0) {
      icerik = `<div class="bos-mesaj">Bu saatte sefer yok</div>`;
    } else {
      icerik = hat.yonler.map(y => {
        if (!y.aktif || y.aktif.length === 0) {
          return `<div class="yon-blok">
            <div class="yon-baslik">${y.yon_adi}</div>
            <div class="bos-mesaj" style="padding:4px 0">Aktif tren yok</div>
          </div>`;
        }
        const trenler = y.aktif.map(t => `
          <div class="mini-tren">
    ...
    card.innerHTML = `
      <div class="hat-card-header">
        ...
        <span style="font-size:.78rem;color:var(--muted)">~${hat.sure||"?"} dk yolculuk</span>
      </div>
      <div class="hat-card-body">${icerik}</div>
    `;
  });
}
